...
function EventEdit() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await eventServices.clearEventImages(id);
      if (selectedImages.length > 0) {
        console.log("🚀 上传图片数量：", selectedImages.length);
        console.log("🖼️ 图片文件名：", selectedImages.map(f => f.name));
        await eventServices.uploadEventImages(id, selectedImages);
      }
